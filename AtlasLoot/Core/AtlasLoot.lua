...
function AtlasLoot_OnVariablesLoaded()
	...
	if(((AtlasLootCharDB.AtlasLootVersion == nil) or (tonumber(AtlasLootCharDB.AtlasLootVersion) < 2002)) and AtlasLootCharDB and AtlasLootCharDB["WishList"] and #AtlasLootCharDB["WishList"]~=0) then
	...
	if((AtlasLootCharDB.AtlasLootVersion == nil) or (tonumber(AtlasLootCharDB.AtlasLootVersion) < 2002)) then
		AtlasLootCharDB.AtlasLootVersion = VERSION_MAJOR..VERSION_MINOR..VERSION_BOSSES;
...
function AtlasLoot_GetLODModule(dataSource)
	if (dataSource=="AtlasLootProjectEpoch") then
		return "AtlasLoot_ProjectEpoch";
	elseif (dataSource=="AtlasLootCraftingPE") then
		return "AtlasLoot_CraftingPE";
...
function AtlasLoot_LoadAllModules()
    ...
    orig, _ = LoadAddOn("AtlasLoot_ProjectEpoch");
    craft, _ = LoadAddOn("AtlasLoot_CraftingPE");
    world, _ = LoadAddOn("AtlasLoot_WorldEvents");
    local flag=0;
	if not orig then
		LoadAddOn("AtlasLoot_ProjectEpoch");
		flag=1;
	end
    if not craft then
		LoadAddOn("AtlasLoot_CraftingPE");
		flag=1;
	end
    if not world then
